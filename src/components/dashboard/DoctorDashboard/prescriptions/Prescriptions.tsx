...
const Prescriptions = () => {
  //  Replace with your actual logic to get the logged-in doctor's ID
  const currentDoctorId = Number(localStorage.getItem("doctor_id"));

  const {
    data: prescriptionsData,
    isLoading,
    error,
  } = prescriptionsAPI.useGetPrescriptionsByDoctorQuery(currentDoctorId, {
    skip: !currentDoctorId, // skip query if no ID yet
          ...
          <table className="min-w-full divide-y divide-gray-200">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {prescriptionsData.map((prescription) => (
                <tr key={prescription.prescription_id} className="hover:bg-gray-50 transition-colors">
