...
const schema: yup.ObjectSchema<RegisterInputs> = yup.object({
  ...
  role: yup.mixed<RoleType>().oneOf(['doctor', 'user'], 'Invalid role').required('Role is required'),
})
                ...
                <div className="grid grid-cols-2 gap-4">
                  <label
                    className={`flex items-center justify-center p-4 border rounded-lg cursor-pointer transition-all ${
                      watch('role') === 'user' ? 'border-teal-500 bg-teal-50' : 'border-gray-300'
                    }`}
                  >
                    ...
                    <div className="flex items-center">
                      <FaUserInjured
                        className={`h-5 w-5 ${
                          watch('role') === 'user' ? 'text-teal-600' : 'text-gray-400'
                        }`}
                      />
                      <span
                        className={`ml-2 ${
                          watch('role') === 'user' ? 'font-medium text-teal-700' : 'text-gray-700'
                        }`}
                      >
                        User
                      </span>
                  ...
                  <label
                    className={`flex items-center justify-center p-4 border rounded-lg cursor-pointer transition-all ${
                      watch('role') === 'doctor' ? 'border-teal-500 bg-teal-50' : 'border-gray-300'
                    }`}
                  >
                    ...
                    <div className="flex items-center">
                      <FaUserMd
                        className={`h-5 w-5 ${
                          watch('role') === 'doctor' ? 'text-teal-600' : 'text-gray-400'
                        }`}
                      />
                      <span
                        className={`ml-2 ${
                          watch('role') === 'doctor' ? 'font-medium text-teal-700' : 'text-gray-700'
                        }`}
                      >
                        Doctor
                      </span>
